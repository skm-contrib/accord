class DynamicAdapt {
  ...
  init() {
    ...
    this.nodes.forEach((node) => {
      ...
      // Перевірка наявності фігурних дужок для пошуку батьківського елемента
      const parentSelectorMatch = dataArray[0]
        .trim()
        .match(/^\{(.+)\}(?:\s*(.+)?)$/);
      if (parentSelectorMatch) {
        // Якщо є фігурні дужки, знайти батьківський елемент відносно поточного елемента
        const parentSelector = parentSelectorMatch[1].trim();
        const childSelector = parentSelectorMatch[2]
          ? parentSelectorMatch[2].trim()
          : null;
        // Знайти батьківський елемент відносно поточного елемента
        const parentElement = node.closest(parentSelector);

        if (parentElement) {
          // Якщо є дочірній селектор, знайти елемент всередині батьківського
          оbject.destination = childSelector
            ? parentElement.querySelector(childSelector)
            : parentElement;
        }
      } else {
        // Звичайний пошук пункту призначення
  ...
  moveTo(place, element, destination) {
    if (!destination) return; // Якщо пункт призначення не знайдено, вихід
...
// Ініціалізація DynamicAdapt з типом "max"
